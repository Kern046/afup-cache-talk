...
export const options = {
    ...
    stages: [
        { duration: '10s', target: 10 }, // Ramp up to 5 virtual users over 30 seconds
        { duration: '30s', target: 80 }, // Ramp to from 5 to 20 virtual users over 90 seconds
        { duration: '30s', target: 800 }, // Ramp to from 5 to 20 virtual users over 90 seconds
        { duration: '2m', target: 800 }, // Stay at 20 virtual users for 3 minutes
...
export default function () {
    const baseUrl = 'https://store.local';

    // Test GET endpoint - fetches a random user
    let getResponse = http.get(`${baseUrl}/`);
    check(getResponse, {
